...
| Battle engine and progression | `bounded` | Trainer and wild battles already cover core damage, type effectiveness, stat stages, trainer AI shaping, EXP, StatExp, level-up learn prompts, payout, blackout, and post-battle continuation. | Add broad move-effect coverage, volatile statuses, major-status rules, battle items, badge/TM reward consequences, special battles, and Red-wide trainer special cases. |
| Inventory, marts, storage, persistence | `bounded` | Viridian Mart buy/sell/quit works, visible items persist, capture can overflow to boxes, schema `8` save/load persists party, boxes, flags, blackout checkpoints, inventory, owned/seen species, and species encounter counts, and legacy saves that predate persisted play-time fields now default those values during decode so `Continue` still opens. | Add whole-game item extraction, general item use, PC deposit/withdraw/release/change-box flows, hidden items, and broader quest-item handling. |
